...
```{r}
remove(mingo, mingo_buyer_addresses, mingo_buyer_join, mingo_buyer_per_year, pills_population, pills_population_full, pills_population_left, pills_population_oops, pills_population_right, county_pills_per_year, county_population_per_year)
...
```{r}
 clean_names(buyer_addresses)
  clean_names(buyer_annual_by_year)
  clean_names(buyer_totals)
  clean_names(state_population_per_year)
  
  #buyer annual has each year individually 2006-2012 while buyer totals has one total for each buyer for the whole 6 years
```
...
```{r}
#Retail Pharmacy in Charleston SC had the most pills sent to it 
```
...
#BM2159932	PRACTITIONER	HARFORD	MD, Rakesh Mathur recived 337,450 pills in 2010
```

**Task**: Which state had the highest rate of total pills per person sent to it over the 2006 to 2012 period?

```{r}
#need to know how many pills were sent to each state 
#buyer_totals
#mutate to create new column with total pills for each state
  #group_by and summarize
  #group all buyer totals together by state and add all the pills (summarize) 
...
buyer_totals_by_state <- buyer_totals %>%
...
buyer_totals_right <- buyer_totals %>%
  right_join(buyer_addresses, by="buyer_dea_no")
#i joined the two tables together to see what the numbers added up.  

anti <- anti_join(buyer_totals, buyer_addresses, by="buyer_dea_no")
#i did an anti join here to see if anything in buyer totals didn't match buyer addresses and there were no rows, so everything in buyer totals shows up in buyer addresses as well
```
...
```{r}
anti_opposite <- anti_join(buyer_addresses, buyer_totals, by="buyer_dea_no")
#there are multiple rows in buyer addresses that are not in buyer totals.  by doing an anti join we can see if any buyer dea number from the first table (buyer addresses) does not show up in the second (buyer totals)
```
...
```{r}
buyers_and_pop <- state_population_per_year %>%
  left_join(buyer_totals_by_state, by="buyer_state")
```
