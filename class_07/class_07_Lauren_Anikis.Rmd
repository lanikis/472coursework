...
#remove(maryland_2012)
#remove(maryland_pills_2012)
#remove(maryland_population_2012)
#remove(maryland_pills_per_year)
#remove(mingo_county_pills_per_year)
#remove(WV_2012)
#remove(WV_pills_2012)
#remove(WV_population_2012)
```
...
```{r}
logan_county_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_county == "LOGAN", buyer_state == "WV") %>%
  group_by(year)
#remove(logan_country_per_year)
```


```{R}
ggplot(logan_county_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="forest green") +
  labs(x="Year", y="Total Pills", title="TBD", subtitle="Logan County, WV opioid shipments 2006-2012", caption="Source: DEA ARCOS database via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)
  
#typed this one out myself, it didn't work, copied from the last lab below, it worked. Can't find a difference????
...
```{r}
  ggplot(logan_county_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="forest green") +
  labs(x="Year", y="Total pills", title="Pills sent to Logan County increased until 2009, afterwhich they experienced a dip with a sharp increase in 2011", subtitle = "Total pills per year shipped to Logan County, WV", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma)
```
...
wv_pills_per_year <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", (buyer_county %in% c("LOGAN", "DODDRIDGE", "FAYETTE", "MINGO", "HARRISON", "JACKSON"))) %>%
  select(buyer_county, dosage_unit, year)
```

```{r}
ggplot(wv_pills_per_year) +
geom_bar(stat="identity", aes(year, dosage_unit, fill=buyer_county)) +
labs(x="year", y="total pills", title="Logan and Mingo counties recieved signifantly more pills than other counties in West Virginia", subtitle="total pills sent to Doddridge, Fayette, Harrison, Jackson, Logan and Mingo Counties", caption="Source: DEA ARCOS database via Washington Post") +
scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
scale_y_continuous(labels = comma) +
...
```{r}
#Doddridge had almost no pills sent to it compared to other counties in WV.  Mingo saw a huge spike in 2008 with approximately 2.5 million more pills sent to Mingo County in 2008 than were sent to any other county at any other time.  Mingo had such a huge difference that when I added Mingo into the mix it changed the scale of the total pills signficantly. 
```
...
census_api_key("83bc120d2ff8d5d4243ad9480b2c02d45028853d")
```
...
```{r}
md_pills_by_county <- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD") %>%
select( buyer_county, buyer_state, dosage_unit, year, countyfips)
```
...
# Final Graphic NEEDS UPDATINGGGGG
