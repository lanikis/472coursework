...
```{r}
#remove(anti)
#remove(anti_opposite)
```

```{r}

#remove(buyer_totals_by_state)
#remove(buyer_totals_left)
#remove(buyer_totals_right)
#remove(buyers_and_pop)
```
...
#this will show me all the pharmacies that recieved pills but were misidentified, how does this happen?

```

```{r}
mingo_md <- county_population(county = "Baltimore", state="MD", key="WaPo")
```

```{r}
mingo_md_pharmacies <- total_pharmacies_county(county = "Baltimore", state="MD", key="WaPo")
```
## Analysis
...
```{r}
mingo_md <- clean_names(mingo_md)
```

```{r}
md_pop_and_buyers <- buyer_annual_by_year %>%
  inner_join(mingo_md, by="year")
```
