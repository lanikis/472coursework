...
```{r}
distributors_address <- mingo %>%
  select(contains("reporter")) %>% 
  mutate(reproter_address_complete = paste0(reporter_address1, " ", reporter_city, ",", reporter_state, " ", reporter_zip))
```
...
#Actavis sent the most through Mckesson to Strosnider
```
...
```{r}
mingo %>%
  group_by(combined_labeler_name, reporter_name, reporter_address1, buyer_name, dosage_unit) %>%
  summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
...
```{r}
#I want to make all reporter_name columns that have the word "CARDINAL" to be named the same way, I would probably use a case_when to rename everything
```
## Submission
